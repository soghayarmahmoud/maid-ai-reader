...
class _PdfReaderPageState extends State<PdfReaderPage> {
  ...
  void _nextSearchResult() {
    _searchService.nextResult();
  }

  void _previousSearchResult() {
    _searchService.previousResult();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return KeyboardListener(
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            // Enhanced search widget with highlighting and navigation
            if (_isSearching)
              PdfSearchWidget(
            ...
            // Search page indicator - shows which pages have results
            StreamBuilder(
              stream: _searchService.searchState,
              builder: (context, snapshot) {
                final session = _searchService.currentSession;
                if (!session.hasResults) return const SizedBox.shrink();
                return SearchPageIndicator(
                  session: session,
                  currentPage: _currentPage,
                  totalPages: _totalPages,
                  onPageTap: (page) {
                    _searchService.navigateToPage(page);
                    _pdfViewerController.jumpToPage(page);
                  },
                );
              },
                  ...
                  GestureDetector(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Show inline search result indicator
                        StreamBuilder(
                          stream: _searchService.searchState,
                          builder: (context, snapshot) {
                            final session = _searchService.currentSession;
                            if (!_isSearching || session.query.isEmpty) {
                              return const SizedBox.shrink();
                            }
                            return InlineSearchResultIndicator(
                              currentResult: session.currentResultIndex,
